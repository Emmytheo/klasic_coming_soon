...
/* Countdown manager. */
  function Countdown () {
    ...
    const perfAvail = (window.performance && typeof window.performance.now === 'function')
    // Shared timer for all countdowns
    function timerCallBack (timestamp) {
      const drawStart = (timestamp < 1e12 // New HTML5 high resolution timer
  ...
  $.extend(Countdown.prototype, {
    ...
    _attachPlugin: function (target, options) {
      ...
      const inst = { options: $.extend({}, this._defaults), _periods: [0, 0, 0, 0, 0, 0, 0] }
    ...
    _optionPlugin: function (target, options, value) {
      target = $(target)
      const inst = target.data(this.propertyName)
      ...
      const timezoneChanged = (inst.options.timezone != options.timezone)
      ...
      if (expired && !inst._expiring) {
        ...
        if (this._hasTarget(target) || inst.options.alwaysExpire) {
          ...
          if (inst.options.expiryText) {
            const layout = inst.options.layout
    ...
    _adjustSettings: function (target, inst, recalc) {
      ...
      if (serverEntry != null) {
        ...
        const serverResult = ($.isFunction(inst.options.serverSync)
      ...
      let timezone = inst.options.timezone
    ...
    _hold: function (target, hold) {
      const inst = $.data(target, this.propertyName)
      if (inst) {
    ...
    _generateHTML: function (inst) {
      ...
      let showCount = 0
      let sigCount = inst.options.significant
      ...
      const showSignificant = [false, false, false, false, false, false, false]
      ...
      const labels = (inst.options.compact ? inst.options.compactLabels : inst.options.labels)
      const whichLabels = inst.options.whichLabels || this._normalLabels
      const showCompact = function (period) {
        const labelsNum = inst.options['compactLabels' + whichLabels(inst._periods[period])]
        return (show[period]
      ...
      const showFull = function (period) {
        const labelsNum = inst.options['labels' + whichLabels(inst._periods[period])]
    ...
    _buildLayout: function (inst, show, layout, compact, significant, showSignificant) {
      const labels = inst.options[compact ? 'compactLabels' : 'labels']
      const whichLabels = inst.options.whichLabels || this._normalLabels
      ...
      for (let i = Y; i <= S; i++) {
        ...
        const re = new RegExp('\\{' + period + '<\\}([\\s\\S]*)\\{' + period + '>\\}', 'g')
    ...
    _determineShow: function (inst) {
      const format = inst.options.format
      const show = []
    ...
    _calculatePeriods: function (inst, show, significant, now) {
      ...
      const until = new Date(inst._now.getTime())
      if (inst._since) {
      ...
      if (show[Y] || show[O]) {
        ...
        const lastNow = plugin._getDaysInMonth(now.getFullYear(), now.getMonth())
        const lastUntil = plugin._getDaysInMonth(until.getFullYear(), until.getMonth())
        ...
        const wasLastDay = (now.getDate() == lastNow)
        const lastDay = plugin._getDaysInMonth(now.getFullYear() + periods[Y],
